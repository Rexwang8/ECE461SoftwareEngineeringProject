...
class Logger:
    def __init__(self, path="log.txt", internalpriority=0):
        self.path = path
    ...
    def log(self, msg, priority=0):
        ...
        with open(self.path, "a") as f:
            if(priority == 1):
                f.write(f"[Python Main Script] Priority: {priority}(info) | Logging at t={time.time()}: {msg}\n")
                f.close()
                return
            if(priority == 2):
                
                f.write(f"[Python Main Script] Priority: {priority}(debug) | Logging at t={time.time()}: {msg}\n")
                f.close()
                return